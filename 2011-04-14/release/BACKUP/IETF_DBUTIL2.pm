package IETF_DBUTIL2;
require Exporter;
@ISA = qw(Exporter);
@EXPORT = qw(init_database get_dbh db_quote db_update db_select db_select_multiple get_name get_email);

use lib '/export/home/mlee/RELEASE/';
use DBI;
use IETF_UTIL;


sub init_database {
   my $request_db = shift;
   $TESTDB = 0;
   $INFORMIX = 1;
   $MYSQL = 2;
   $CURRENT_DATE = "CURRENT_DATE"; # "TODAY" for Informix, "CURRENT_DATE" for MySQL
   $CONVERT_SEED = 1; # To convert date format to fit into the current database eng
   $is_null = " = '' ";
   if ($ENV{OSTYPE}  =~ /linux/) {
     $db_mode = $MYSQL;
   } else {
     if ($request_db == $MYSQL) {
	  $db_mode = $MYSQL
     } else {
	  $db_mode = $INFORMIX;
     }
   }
   if ($db_mode == $MYSQL) {
      $ENV{"DBNAME"} = "ietf";
   } elsif ($db_mode == $INFORMIX) {
      $ENV{"DBPATH"} = "/usr/informix/databases";
      $ENV{"DBNAME"} = "people";
      $CURRENT_DATE = "TODAY";
      $CONVERT_SEED = 2;
      $is_null = " is NULL ";
   } else {
      $ENV{"DBPATH"} = "/export/home/mlee/database";
      $ENV{"DBNAME"} = "testdb";
      $CURRENT_DATE = "TODAY";
      $CONVERT_SEED = 2;
      $is_null = " is NULL ";
   }
   return ($db_mode,$CURRENT_DATE,$CONVERT_SEED,$is_null);
}

sub get_dbh {
   $dbname = $ENV{"DBNAME"};
   my $dbh;
   if ($dbname =~ /ietf/) {
      $dbh = DBI->connect("dbi:mysql:$dbname:10.27.6.20","mlee","sunnyohm");
   } else {
      $dbh = DBI->connect("dbi:Informix:$dbname");
   }
   return $dbh;
}


sub db_quote {
  my @list = @_;
  my $dbh = get_dbh();
  for ($loop=0;$loop<=$#list;$loop++) {
    $list[$loop] =~ s/'/''/g;
    if ($ENV{"DBNAME"} =~ /ietf/) {
	  $list[$loop] = "'" . rm_tr($list[$loop]);
	  $list[$loop] .= "'";
	} else {
      $list[$loop] = $dbh->quote(rm_tr($list[$loop]));
    }
  }
  $dbh->disconnect();
   if ($#list == 0) {
     return $list[0];
   } else {
     return @list;
   }
}

sub db_update {
   my $sqlStr = shift;
   my $dbh = get_dbh();
   my $sth = $dbh->prepare($sqlStr) || return 0;
   $sth->execute();
   #$sth->execute() || return 0;
   $dbh->disconnect();
   return 1;
}

sub db_select {
   my $sqlStr = shift;
   my @list;
   my $dbh = get_dbh();
   my $sth = $dbh->prepare($sqlStr) || print "$sqlStr\n";
   $sth->execute();
   @list = $sth->fetchrow_array();
   #$dbh->disconnect();
   if ($#list == 0) {
     return $list[0];
   } else {
     return @list;
   }
}

sub db_select_multiple {
   my $sqlStr = shift;
   my $dbh = get_dbh();
   my $sth = $dbh->prepare($sqlStr);
   $sth->execute();
   my @nList;
   while (my @row = $sth->fetchrow_array()) {
      push @nList, [ @row ];
   }
   return @nList;
}

sub get_name {
   my $person_or_org_tag = shift;
   my $reverse = shift;
   my ($firstname,$lastname) = rm_tr(db_select("select firstname,lastname from print_name where person_or_org_tag = $person_or_org_tag"));
   return "$lastname, $firstname" if ($reverse);
   return "$firstname $lastname";
}

sub get_email {
   my $person_or_org_tag = shift;
   my $ret_val = rm_tr(db_select("select email_address from email_addresses where person_or_org_tag = $person_or_org_tag"));                     

   return $ret_val;
}


