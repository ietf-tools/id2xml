...
sub get_wg_individual {
  ...
  $html_txt .= get_new_returning($sqlStr,"NEW",$valid_sql,$hd_num);
  $html_txt .= get_new_returning($sqlStr,"RET",$valid_sql,$hd_num);
  $html_txt .= $table_footer;
  return $html_txt;
}
...
sub get_via_rfc_ad {
  ...
  if ($type eq "AD") {
    $hd_num = "${indexCnt}.2";
    $heading .= "<h3>$hd_num Individual Submissions Via AD";
    $sqlStr .= $not_via_rfc_where;
    $valid_sql .= $not_via_rfc_where;
  } else {
    $hd_num = "${indexCnt}.3";
    $heading .= "<h3>$hd_num Individual Submissions Via RFC Editor";
