...
sub get_wg_individual {
  ...
  if ($type eq "IND") {
    ...
    if ($group_type eq "DA") {
      ...
      $ret_txt .= get_via_rfc_ad($sqlStr,"AD",$valid_sql);
  ...
  return "" unless ($valid); 
  my $ret_txt = "$gap $heading\n";
  $ret_txt .= get_new_returning($sqlStr,"NEW",$valid_sql);
  $ret_txt .= get_new_returning($sqlStr,"RET",$valid_sql);
  $ret_txt .= "\n";
  return $ret_txt;
}
...
sub get_new_returning {
  ...
  for $array_ref (@List) {
   ...
   $ret_txt .= qq {$gap $gap $gap o $one_line
           <$filename>
};
