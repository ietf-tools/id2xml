...
sub input_screen {
   ...
   $sqlStr = qq{
   select p.first_name,p.last_name,p.person_or_org_tag from person_or_org_info p, area_directors a, acronym m
...
sub update_internal_db {
  ...
  if ($update_tag) {
   ...
   values 
   ($ballot_id,$primary_flag,$group_flag,$id_document_tag,$status_date,$token_name,$token_email,$token_name,$agenda,$rfc_flag,$area_acronym,$mark_by,$assigned_to,999999,$CURRENT_DATE,2)
	 };
  }
  print "FATAL ERROR:\n$sqlStr" unless (db_update($sqlStr));
}
...
|;
   ...
   $sqlStr = qq {
		 ...
		 WHERE groups_ietf.group_acronym_id = $group_acronym_id
		   ...
		   AND B.person_or_org_tag 			= person_or_org_info.person_or_org_tag
   };
#   die "$sqlStr\n";
