...
sub view_id {
   ...
   for $array_ref (@commentList) {
	  ...
	  <form>
	  <td>
	  <input type="button" value="View Detail" onClick="window.open('${program_name}?command=view_comment&loginid=$loginid&id=$id',null,'height=250,width=500,status=no,toolbar=no,menubar=no,location=no,resizable=yes');">
	  </td>
	  </form>
	  $button_str
	  </tr>
...
sub email_to_AD {
   return 1 if (my_defined($devel) or $db_mode == $INFORMIX);  ## To be changed if migrate to MySQL
