...
cur_state,token_email,email_display,note,acronym
from id_internal id, acronym a
where group_flag < 15 AND primary_flag = 1 AND cur_state <> 41 AND cur_state <> 99 AND id.area_acronym_id = a.acronym_id
...
for $array_ref (@List) {
   ...
   if ($cur_state != $pCur_state) {
      ...
      my $state_name = rm_tr(db_select("select document_state_val from ref_doc_states_new where document_state_id = $cur_state"));
...
$line
   ...
   from id_internal id, acronym a
   where ballot_id = $ballot_id AND (primary_flag = 0 OR primary_flag is null)  AND id.area_acronym_id = a.acronym_id
