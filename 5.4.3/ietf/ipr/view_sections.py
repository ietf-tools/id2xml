...
from ietf.ipr.models import HolderIprDisclosure,GenericIprDisclosure,ThirdPartyIprDisclosure,NonDocSpecificIprDisclosure
...
def section_list_for_ipr(ipr):
    # new logic
    if isinstance(ipr,HolderIprDisclosure):
        return section_table["specific"]
    if isinstance(ipr,GenericIprDisclosure):
        return section_table["generic"]
    if isinstance(ipr,ThirdPartyIprDisclosure):
        return section_table["third-party"]
    if isinstance(ipr,NonDocSpecificIprDisclosure):
        return section_table["specific"]
