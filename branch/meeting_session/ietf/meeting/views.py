...
from ietf.idtracker.models import IRTF, IETFWG, WGSecretary, WGChair, SessionRequestActivities
...
def show_schedule(request):
    ...
    return render("meeting/schedule.html", {'meeting_num': meeting_num, 'unscheduled_groups':unscheduled_groups, 'scheduled_groups':scheduled_groups, 'scheduled_irtf': scheduled_irtf, 'unscheduled_irtf': unscheduled_irtf} )
...
def schedule_group(request, meeting_num=None, group_id=None):
    ...
    if 'not_meeting' in request.POST:
        ...
        return render("meeting/schedule_not_meeting.html", {'meeting_num': meeting_num, 'group': group, 'person': person} )
        ...
        if meetingform.is_valid():
            ...
            if session_id == 'new':
                ...
                SessionRequestActivities(group_acronym_id=group.group_acronym_id, activity="New session was requested", meeting_num=meeting_num, person=person).save()
            else:
                meeting_session = get_object_or_404(WgMeetingSession, session_id=session_id)
                SessionRequestActivities(group_acronym_id=group.group_acronym_id, activity="Updated session was requested", meeting_num=meeting_num, person=person).save()
            ...
            # Todo: Detect irtf group
            meeting_session.irtf=False
            ...
            return render("meeting/schedule_confirm.html", {'meeting_num': meeting_num, 'group': group, 'session': meeting_session})
        else:
            # Redisplay form with error messages
            return render("meeting/schedule_group.html", {'meeting_num': meeting_num, 'group': group, 'is_new': True, 'meetingform': meetingform} )
        ...
        try:
            ...
            return render("meeting/schedule_group.html", {'meeting_num': meeting_num, 'group': group, 'meetingform': meetingform, 'last_session': scheduling.last_group_session(meeting_num, group_id)} )
        ...
        return render("meeting/schedule_cancel_meeting.html", {'meeting_num': meeting_num, 'group': group, 'person': person} )
        ...
        try:
            ...
            return render("meeting/schedule_group_review.html", {'meeting_num': meeting_num, 'group': group, 'session': Session, 'session_activity': session_activity, 'conflicts': conflicts} )

        # Else group is not scheduled yet
        except ObjectDoesNotExist:
            meetingform = MeetingSession({'group_acronym_id': group.group_acronym_id, 'session_id': 'new'})
            return render("meeting/schedule_group.html", {'meeting_num': meeting_num, 'group': group, 'is_new': True, 'meetingform': meetingform, 'conflicts': conflicts, 'last_session': scheduling.last_group_session(meeting_num, group_id) } )
