...
sub email_to_AD {
   ...
   if ($id_status_log =~ /A new comment added/) {
     ...
     open MAIL, "| /usr/lib/sendmail ietf.tracker.archive\@googlemail.com" or return 0;
...
sub get_filename_set {
   ...
   for $array_ref (@List_file) {
     ...
     if ($type == 5) {
         ...
         $filename2 = qq{<a href="https://merlot.tools.ietf.org/cgi-bin/idtracker.cgi?loginid=$loginid&command=view_id&dTag=$tag&rfc_flag=$rfc_flag&ballot_id=$ballot_id">$filename2</a>
};
