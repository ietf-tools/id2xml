...
def forward(apps, schema_editor):
    ...
    for stream in ('ietf','irtf','ise','iab'):
        idexists['draft-stream-' + stream] = State.objects.create(
            ...
            name = 'I-D Exists',
            used = True,
            desc = 'The %s stream has not started processing this draft, or has stopped processing it without requesting publicastion.' % stream.upper(),
            order = 0,
        )
    ...
    for doc in Document.objects.filter(type='draft'):
