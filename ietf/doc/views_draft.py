...
from ietf.doc.models import (
    Document, DocAlias, RelatedDocument, State, StateType, DocEvent,
    ConsensusDocEvent, TelechatDocEvent, WriteupDocEvent, IESG_SUBSTATE_TAGS)
...
class ChangeStateForm(forms.Form):
    state = forms.ModelChoiceField(
        State.objects.filter(used=True, type="draft-iesg"),
        empty_label=None,
        required=True)
    substate = forms.ModelChoiceField(
        DocTagName.objects.filter(slug__in=IESG_SUBSTATE_TAGS), required=False)
    comment = forms.CharField(
        widget=forms.Textarea, required=False, strip=False)
    ...
    def clean(self):
        ...
        if state == prev and tag == prev_tag:
            self._errors['comment'] = ErrorList([
                u'State not changed. Comments entered will be lost with no state change. Please go back and use the Add Comment feature on the history tab to add comments without changing state.'
            ])
        return retclean
...
def change_state(request, name):
    ...
    if (not doc.latest_event(
            type="started_iesg_process")) or doc.get_state_slug() == "expired":
        raise Http404
...
>>>>>>> .merge-right.r13947
            ...
            if new_state != prev_state or set(new_tags) != set(prev_tags):
                ...
                if new_state.slug == "approved" and new_tags == [] and has_role(
                        request.user, "Area Director"):
                    email_ad_approved_doc(request, doc, comment)

                if prev_state and prev_state.slug in (
                        "ann",
                        "rfcqueue") and new_state.slug not in ("rfcqueue",
                                                               "pub"):
                    email_pulled_from_rfc_queue(request, doc, comment,
                ...
                if new_state.slug == "lc-req":
                    ...
                    return render(request,
                                  ...
                                  dict(doc=doc, url=doc.get_absolute_url()))
        ...
        form = ChangeStateForm(initial=dict(
            state=state.pk if state else None, substate=t[0].pk
            if t else None))
        form.docname = name
    ...
    hists = doc.history_set.exclude(
        states=doc.get_state("draft-iesg")).order_by('-time')[:1]
    if hists:
    ...
    if not doc.latest_event(type="sent_ballot_announcement"):
        if next_states and next_states.filter(slug="iesg-eva"):
            to_iesg_eval = State.objects.get(
                used=True, type="draft-iesg", slug="iesg-eva")
...
class ChangeIanaStateForm(forms.Form):
    ...
    def __init__(self, state_type, *args, **kwargs):
        ...
        choices = State.objects.filter(
            used=True, type=state_type).order_by("order").values_list(
                "pk", "name")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if new_state != prev_state:
                ...
                events = [
                    add_state_change_event(doc, request.user.person,
                                           prev_state, new_state)
                ]
        ...
        form = ChangeIanaStateForm(
            state_type,
            initial=dict(state=prev_state.pk if prev_state else None))

    return render(request, 'doc/draft/change_iana_state.html',
                  dict(form=form, doc=doc))


class ChangeStreamForm(forms.Form):
    stream = forms.ModelChoiceField(
        StreamName.objects.exclude(slug="legacy"), required=False)
    comment = forms.CharField(
        widget=forms.Textarea, required=False, strip=False)
...
def change_stream(request, name):
    ...
    if not (has_role(request.user, ("Area Director", "Secretariat")) or
            (request.user.is_authenticated and Role.objects.filter(
                name="chair",
                group__acronym__in=StreamName.objects.values_list(
                    "slug", flat=True),
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if new_stream != old_stream:
                ...
                e = DocEvent(
                    doc=doc, rev=doc.rev, by=login, type='changed_document')
                ...
                if comment:
                    c = DocEvent(
                        doc=doc, rev=doc.rev, by=login, type="added_comment")
                    c.desc = comment
                    c.save()
                    events.append(c)
...
class ReplacesForm(forms.Form):
    ...
    def clean_replaces(self):
        for d in self.cleaned_data['replaces']:
            ...
            if d.document.type_id == "draft" and d.document.get_state_slug(
            ) == "rfc":
...
def replaces(request, name):
    ...
    if doc.type_id != 'draft':
        raise Http404
    if not (has_role(request.user,
                     ("Secretariat", "Area Director", "WG Chair", "RG Chair",
                      "WG Secretary", "RG Secretary")) or
            is_authorized_in_doc_stream(request.user, doc)):
        return HttpResponseForbidden(
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if new_replaces != old_replaces:
                events = set_replaces_for_document(
                    request,
                    doc,
                    new_replaces,
                    by=by,
                    email_subject="%s replacement status updated by %s" %
                    (doc.name, by),
                    comment=comment)
...
class SuggestedReplacesForm(forms.Form):
    replaces = forms.ModelMultipleChoiceField(
    ...
    def __init__(self, suggested, *args, **kwargs):
        ...
        self.fields["replaces"].queryset = self.fields[
            "replaces"].queryset.filter(pk__in=pks)
...
def review_possibly_replaces(request, name):
    ...
    if doc.type_id != 'draft':
        raise Http404
    if not (has_role(request.user, ("Secretariat", "Area Director")) or
            is_authorized_in_doc_stream(request.user, doc)):
        return HttpResponseForbidden(
            "You do not have the necessary permissions to view this page")
        ...
        if form.is_valid():
            ...
            events.append(
                DocEvent.objects.create(
                    ...
                    rev=doc.rev,
                    by=by,
                    type="reviewed_suggested_replaces",
                    desc="Reviewed suggested replacement relationships: %s" %
                    ", ".join(d.name for d in suggested)))
            RelatedDocument.objects.filter(
                source=doc,
                target__in=suggested,
                relationship__slug='possibly-replaces').delete()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if new_level != old_level:
                ...
                de = doc.latest_event(
                    ConsensusDocEvent, type="changed_consensus")
                prev_consensus = de and de.consensus
                if not prev_consensus and doc.intended_std_level_id in (
                        "std", "ds", "ps", "bcp"):
                    ce = ConsensusDocEvent(
                        doc=doc,
                        rev=doc.rev,
                        by=login,
                        type="changed_consensus")
                    ce.consensus = True
                    ce.desc = "Changed consensus to <b>%s</b> from %s" % (
                        nice_consensus(True), nice_consensus(prev_consensus))
                    ce.save()
                    events.append(ce)
...
class EditInfoForm(forms.Form):
    ...
    ad = forms.ModelChoiceField(
        Person.objects.filter(
            ...
            role__group__type='area').order_by('name'),
        label="Responsible AD",
        empty_label="(None)",
        required=True)
    create_in_state = forms.ModelChoiceField(
        State.objects.filter(
            used=True, type="draft-iesg", slug__in=("pub-req", "watching")),
        empty_label=None,
        required=False)
    ...
    note = forms.CharField(
        widget=forms.Textarea, label="IESG note", required=False, strip=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        # telechat choices
        dates = [
            d.date for d in TelechatDate.objects.active().order_by('date')
        ]
        ...
        self.standard_fields = [
            x for x in self.visible_fields()
            if x.name not in ('returning_item', )
        ]
...
def to_iesg(request, name):
    ...
    if doc.get_state_slug('draft') == "expired" or doc.get_state_slug(
            'draft-iesg') == 'pub-req':
        raise Http404
    ...
    target_map = {'draft-iesg': 'iesg', 'draft-stream-ietf': 'wg'}
    ...
    if not doc.shepherd:
        warn['shepherd'] = True
    shepherd_writeup = doc.latest_event(
    ...
    if request.method == 'POST':
        ...
        if request.POST.get("confirm", ""):
            ...
            if not doc.get_state("draft-iesg"):
                ...
                e.doc = doc
                e.rev = doc.rev
                e.desc = "IESG process started in state <b>%s</b>" % target_state[
                    'iesg'].name
                e.save()
                events.append(e)
            ...
            if not doc.notify == notify:
                doc.notify = notify
                changes.append("State Change Notice email list changed to %s" %
                               doc.notify)
...
# Get the last available writeup
            ...
            send_mail(
                ...
                subject="Publication has been requested for %s-%s" % (doc.name,
                                                                      doc.rev),
                template="doc/submit_to_iesg_email.txt",
                context=dict(
                    doc=doc,
                    by=by,
                    url="%s%s" % (settings.IDTRACKER_BASE_URL,
...
def edit_info(request, name):
        ...
        if form.is_valid():
            ...
            if new_document:
                ...
                if replaces:
                    ...
                    e.doc = doc
                    e.rev = doc.rev
                    e.desc = "Earlier history may be found in the Comment Log for <a href=\"%s\">%s</a>" % (
                ...
                e.doc = doc
                e.rev = doc.rev
                e.desc = "IESG process started in state <b>%s</b>" % doc.get_state(
                    "draft-iesg").name
                e.save()
                events.append(e)
            ...
            if doc.group.type_id in ("individ", "area"):
                ...
                if r["area"] != doc.group:
                    if r["area"].type_id == "area":
                        changes.append(
                            u"Assigned to <b>%s</b>" % r["area"].name)
                    else:
            ...
            # Todo - chase this
            e = update_telechat(request, doc, by, r['telechat_date'],
                                r['returning_item'])
    ...
    if not new_document:
        form.standard_fields = [
            x for x in form.standard_fields if x.name != "create_in_state"
        ]
    if doc.group.type_id not in ("individ", "area"):
        form.standard_fields = [
            x for x in form.standard_fields if x.name != "area"
        ]
    ...
    return render(request, 'doc/draft/edit_info.html',
                  dict(
                      ...
                      form=form,
                      user=request.user,
                      ballot_issued=doc.latest_event(
                          type="sent_ballot_announcement")))
...
def resurrect(request, name):
    ...
    e = doc.latest_event(type__in=('requested_resurrect',
                                   "completed_resurrect"))
    ...
    if request.method == 'POST':
        if resurrect_requested_by:
            email_resurrection_completed(
                request, doc, requester=resurrect_requested_by)
        ...
        doc.set_state(
            State.objects.get(used=True, type="draft", slug="active"))
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if new_note != doc.note:
                ...
                c = DocEvent(
                    type="added_comment", doc=doc, rev=doc.rev, by=login)
                c.desc = log_message
                c.save()
...
class ShepherdWriteupUploadForm(forms.Form):
    content = forms.CharField(
        widget=forms.Textarea,
        label="Shepherd writeup",
        help_text="Edit the shepherd writeup.",
        required=False,
        strip=False)
    txt = forms.FileField(
        label=".txt format",
...
def edit_shepherd_writeup(request, name):
    ...
    can_edit_shepherd_writeup = (
        can_edit_stream_info or
        (doc.shepherd and user_is_person(request.user, doc.shepherd.person)) or
    ...
    if request.method == 'POST':
        if "submit_response" in request.POST:
            ...
            if form.is_valid():
                ...
                if (iesg_state or
                    (stream_state and stream_state.slug == 'sub-pub')):
                    e.desc = writeup
                else:
                ...
                return redirect(
                    'ietf.doc.views_doc.document_main', name=doc.name)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if form.cleaned_data['shepherd'] != doc.shepherd:
                ...
                if doc.shepherd and (
                        doc.shepherd.formatted_email() not in doc.notify):
                    addrs = doc.notify
                    if addrs:
                        addrs += ', '
                    addrs += doc.shepherd.formatted_email()
                    events.append(
                        make_notify_changed_event(
                            request, doc, request.user.person, addrs, c.time))
                    doc.notify = addrs
                ...
                messages.info(
                    request,
                    "The selected shepherd was already assigned - no changes have been made."
                )
...
class ChangeShepherdEmailForm(forms.Form):
    shepherd = forms.ModelChoiceField(
        queryset=Email.objects.all(), label="Shepherd email", empty_label=None)

    def __init__(self, *args, **kwargs):
        super(ChangeShepherdEmailForm, self).__init__(*args, **kwargs)
        self.fields[
            "shepherd"].queryset = self.fields["shepherd"].queryset.filter(
                person__email=self.initial["shepherd"]).distinct()
...
def change_shepherd_email(request, name):
    ...
    can_edit_stream_info = is_authorized_in_doc_stream(request.user, doc)
    is_shepherd = user_is_person(request.user, doc.shepherd and
                                 doc.shepherd.person)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if form.cleaned_data['shepherd'] != doc.shepherd:
                ...
                messages.info(
                    request,
                    "The selected shepherd address was already assigned - no changes have been made."
                )
...
class ConsensusForm(forms.Form):
    consensus = forms.ChoiceField(
        choices=(("Unknown", "Unknown"), ("Yes", "Yes"), ("No", "No")),
        required=True,
        label=
        "When published as an RFC, should the consensus boilerplate be included?"
    )
...
def edit_consensus(request, name):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if form.cleaned_data["consensus"] != prev_consensus:
                ...
                e.consensus = {
                    ...
                    "Yes": True,
                    "No": False
                }[form.cleaned_data["consensus"]]
                if not e.consensus and doc.intended_std_level_id in (
                        "std", "ds", "ps", "bcp"):
                    return HttpResponseForbidden(
                        "BCPs and Standards Track documents must include the consensus boilerplate"
                    )

                e.desc = "Changed consensus to <b>%s</b> from %s" % (
                    nice_consensus(e.consensus),
        ...
        form = ConsensusForm(initial=dict(
            consensus=nice_consensus(prev_consensus)))
...
def request_publication(request, name):
    ...
    doc = get_object_or_404(
        Document, type="draft", name=name, stream__in=("iab", "ise", "irtf"))

    if not is_authorized_in_doc_stream(request.user, doc):
        return HttpResponseForbidden(
            "You do not have the necessary permissions to view this page")

    consensus_event = doc.latest_event(
        ConsensusDocEvent, type="changed_consensus")
    ...
    next_state = State.objects.get(
        used=True, type="draft-stream-%s" % doc.stream.slug, slug="rfc-edit")
    ...
    if request.method == 'POST' and not request.POST.get("reset"):
        ...
        subject = "%s: '%s' to %s (%s-%s.txt)" % (action, doc.title,
                                                  std_level_prompt(doc),
                                                  doc.name, doc.rev)
    ...
    return render(
        request,
        'doc/draft/request_publication.html',
        dict(
            form=form,
            doc=doc,
            message=m,
            next_state=next_state,
            consensus_filled_in=(True if (doc.stream_id and
                                          doc.stream_id == 'ietf') else
                                 (consensus_event != None and
                                  consensus_event.consensus != None)), ), )


class AdoptDraftForm(forms.Form):
    group = forms.ModelChoiceField(
        queryset=Group.objects.filter(type__in=["wg", "rg"],
                                      state="active").order_by(
                                          "-type", "acronym"),
        required=True,
        empty_label=None)
    newstate = forms.ModelChoiceField(
        queryset=State.objects.filter(
            type__in=['draft-stream-ietf', 'draft-stream-irtf'],
            used=True).exclude(
    ...
    weeks = forms.IntegerField(
        required=False, label="Expected weeks in adoption state")
    ...
    def __init__(self, *args, **kwargs):
        ...
        if has_role(user, "Secretariat"):
            state_choices = State.objects.filter(
                type__in=['draft-stream-ietf', 'draft-stream-irtf'],
                used=True).exclude(
                    slug__in=settings.GROUP_STATES_WITH_EXTRA_PROCESSING)
            ...
            group_ids = list(
                Group.objects.filter(type="rg", state="active").values_list(
                    'id', flat=True))
            group_ids.extend(
                list(
                    Group.objects.filter(
                        ...
                        role__name__in=("chair", "delegate", "secr"))
                    .values_list('id', flat=True)))
            self.fields["group"].queryset = self.fields[
                "group"].queryset.filter(id__in=group_ids).distinct()
            state_choices = State.objects.filter(
                type='draft-stream-irtf', used=True).exclude(
                    slug__in=settings.GROUP_STATES_WITH_EXTRA_PROCESSING)
        else:
            self.fields["group"].queryset = self.fields[
                "group"].queryset.filter(
                    role__person__user=user,
                    role__name__in=("chair", "delegate", "secr")).distinct()
            state_choices = State.objects.filter(
                type='draft-stream-ietf', used=True).exclude(
                    slug__in=settings.GROUP_STATES_WITH_EXTRA_PROCESSING)

        self.fields['group'].choices = [(g.pk, '%s - %s' % (g.acronym, g.name))
                                        for g in self.fields["group"].queryset]
        self.fields['newstate'].choices = [('', '-- Pick a state --')]
        self.fields['newstate'].choices.extend(
            [(x.pk, x.name + " (IETF)") for x in state_choices
             if x.type_id == 'draft-stream-ietf'])
        self.fields['newstate'].choices.extend(
            [(x.pk, x.name + " (IRTF)") for x in state_choices
    ...
    def clean_newstate(self):
        ...
        if (newstate.type_id == 'draft-stream-ietf') and (
                group.type_id == 'rg'):
            raise forms.ValidationError(
                'Cannot assign IETF WG state to IRTF group')
        elif (newstate.type_id == 'draft-stream-irtf') and (
                group.type_id == 'wg'):
            raise forms.ValidationError(
                'Cannot assign IRTF RG state to IETF group')
        else:
            return newstate
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if doc.stream != new_stream:
                e = DocEvent(
                    type="changed_stream", doc=doc, rev=doc.rev, by=by)
            ...
            events.append(
                make_notify_changed_event(request, doc, by, new_notify))
...
class ChangeStreamStateForm(forms.Form):
    new_state = forms.ModelChoiceField(
        queryset=State.objects.filter(used=True), label='State')
    weeks = forms.IntegerField(label='Expected weeks in state', required=False)
    comment = forms.CharField(
        widget=forms.Textarea,
        required=False,
        help_text="Optional comment for the document history.",
        strip=False)
    tags = forms.ModelMultipleChoiceField(
        queryset=DocTagName.objects.filter(used=True),
        widget=forms.CheckboxSelectMultiple,
        required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if doc.group:
            unused_states = doc.group.unused_states.values_list(
                "pk", flat=True)
    ...
    def clean_new_state(self):
        ...
        if new_state.slug == 'sub-pub' and not self.can_set_sub_pub:
            raise forms.ValidationError(
                'You may not set the %s state using this form. Use the "Submit to IESG for publication" button on the document\'s main page instead. If that button does not appear, the document may already have IESG state. Ask your Area Director or the Secretariat for help.'
                % new_state.name)
        return new_state
...
def next_states_for_stream_state(doc, state_type, current_state):
    ...
    if current_state:
        ...
        if doc.stream_id == "ietf" and doc.group:
            transitions = doc.group.groupstatetransitions_set.filter(
                state=current_state)
            if transitions:
        ...
        states = State.objects.filter(
            used=True, type=state_type).order_by('order')
        if states:
...
def change_stream_state(request, name, state_type):
    ...
    can_set_sub_pub = has_role(
        request.user,
        ('Secretariat', 'Area Director')) or (prev_state and
                                              prev_state.slug == 'sub-pub')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if new_state != prev_state:
                ...
                email_stream_state_changed(request, doc, prev_state, new_state,
                                           by, comment)
            ...
            if existing_tags != new_tags:
                ...
                e = DocEvent(
                    type="changed_document", doc=doc, rev=doc.rev, by=by)
                added_tags = new_tags - existing_tags
                removed_tags = existing_tags - new_tags
                l = []
                if added_tags:
                    l.append(u"Tag%s %s set." %
                             (pluralize(added_tags),
                              ", ".join(t.name for t in added_tags)))
                if removed_tags:
                    l.append(u"Tag%s %s cleared." %
                             (pluralize(removed_tags),
                              ", ".join(t.name for t in removed_tags)))
                e.desc = " ".join(l)
                e.save()
                events.append(e)

                email_stream_tags_changed(request, doc, added_tags,
                                          removed_tags, by, comment)
            ...
            if events:
                ...
                form.add_error(
                    None,
                    "No change in state or tags found, and no comment provided -- nothing to do."
                )
    else:
        form = ChangeStreamStateForm(
            initial=dict(
                new_state=prev_state.pk if prev_state else None,
                tags=doc.tags.all()),
            doc=doc,
            state_type=state_type,
            can_set_sub_pub=can_set_sub_pub,
