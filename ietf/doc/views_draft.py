...
class SuggestedReplacesForm(forms.Form):
    replaces = forms.ModelMultipleChoiceField(queryset=DocAlias.objects.all(),
                                              label="Suggested replaces", required=False, widget=forms.CheckboxSelectMultiple,
                                              help_text="Verify the checked documents are indeed replaced, or uncheck them")
...
def review_suggested_replaces(request, name):
    doc = get_object_or_404(Document, docalias__name=name)
    if doc.type_id != 'draft':
        raise Http404
    if not (has_role(request.user, ("Secretariat", "Area Director"))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            RelatedDocument.objects.filter(source=doc, target__in=suggested).delete()
    ...
    return render(request, 'doc/draft/review_suggested_replaces.html',
                  dict(form=form,
                       doc=doc,
                   ))
