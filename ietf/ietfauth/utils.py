    ...
    if key not in user.roles_check_cache:
        ...
        role_qs = {
            ...
            Q(person=person,
              ...
              group__state="active"),
            "Secretariat":
            Q(person=person, name="secr", group__acronym="secretariat"),
            "IAB":
            Q(person=person, name="member", group__acronym="iab"),
            "IANA":
            Q(person=person, name="auth", group__acronym="iana"),
            "RFC Editor":
            Q(person=person, name="auth", group__acronym="rfceditor"),
            "ISE":
            Q(person=person, name="chair", group__acronym="ise"),
            "IAD":
            Q(person=person, name="admdir", group__acronym="ietf"),
            "IETF Chair":
            Q(person=person, name="chair", group__acronym="ietf"),
            "IETF Trust Chair":
            Q(person=person, name="chair", group__acronym="ietf-trust"),
            "IRTF Chair":
            Q(person=person, name="chair", group__acronym="irtf"),
            "IAB Chair":
            Q(person=person, name="chair", group__acronym="iab"),
            "IAB Executive Director":
            Q(person=person, name="execdir", group__acronym="iab"),
            "IAB Group Chair":
            Q(person=person,
              name="chair",
              group__type="iab",
              group__state="active"),
            "IAOC Chair":
            Q(person=person, name="chair", group__acronym="iaoc"),
            "WG Chair":
            Q(person=person,
              name="chair",
              group__type="wg",
            ...
            Q(person=person,
              name="chair",
              group__type="rg",
              group__state__in=["active", "proposed"]),
            "RG Secretary":
            Q(person=person,
              name="secr",
              group__type="rg",
            ...
            Q(
                ...
                group__state="active", ),
            "Reviewer":
            Q(person=person, name="reviewer", group__state="active"),
        ...
        user.roles_check_cache[key] = bool(
            Role.objects.filter(filter_expr).exists())
...
def role_required(*role_names):
    ...
    return passes_test_decorator(
        ...
        "Restricted to role%s %s" % ("s" if len(role_names) != 1 else "",
                                     ", ".join(role_names)))
...
def is_authorized_in_doc_stream(user, doc):
    ...
    if (not doc.stream or doc.stream.slug == "ietf") and has_role(
            user, ["Area Director"]):
        return True
    ...
    return Role.objects.filter(
        Q(name__in=("chair", "secr", "delegate", "auth"), person__user=user) &
        group_req).exists()
