...
def find_ads_for_meeting(meeting):
    ads = []
    meeting_time = datetime.datetime.combine(meeting.date,
                                             datetime.time(0, 0, 0))
        ...
        if history and history != g:
            ...
            if history.state_id == "active":
                for x in history.rolehistory_set.filter(
                        name="ad", group__type='area').select_related(
                            'group', 'person', 'email'):
                    #print "xh[%u]: %s" % (num, x)
                    ads.append(x)
        else:
...
def get_pseudo_areas():
    return Group.objects.filter(
        Q(state="active", name="IRTF") | Q(state="active", name="IETF") | Q(
            state="active", type="area")).order_by('acronym')


# get list of all areas, + IRTF.
def get_areas():
    return Group.objects.filter(
        Q(state="active", name="IRTF") | Q(state="active",
                                           type="area")).order_by('acronym')


# get list of areas that are referenced.
def get_area_list_from_sessions(assignments, num):
    return assignments.filter(
        timeslot__type='Session',
...
def build_all_agenda_slices(meeting):
    ...
    for ts in meeting.timeslot_set.filter(type__in=[
            'session',
    ]).order_by('time', 'name'):
        ymd = ts.time.date()
        ...
        if ymd in date_slices:
            if [ts.time, ts.time + ts.duration
                ] not in date_slices[ymd]:  # only keep unique entries
...
def get_modified_from_assignments(assignments):
    return assignments.aggregate(
        Max('timeslot__modified'))['timeslot__modified__max']
...
def get_wg_list(assignments):
    ...
    return Group.objects.filter(acronym__in=set(wg_name_list)).order_by(
        'parent__acronym', 'acronym')


def get_meetings(num=None, type_in=[
        'ietf',
]):
...
def get_meeting(num=None, type_in=[
        'ietf',
]):
...
def preprocess_assignments_for_agenda(assignments_queryset, meeting):
    ...
    assignments_queryset = assignments_queryset.select_related(
        ...
        "session",
        "session__group",
        "session__group__charter",
        "session__group__charter__group", ).prefetch_related(
            Prefetch(
                "session__materials",
                queryset=Document.objects.exclude(
                    states__type=F("type"),
                    states__slug='deleted').select_related("group").order_by(
                        "sessionpresentation__order"),
    ...
    groups = [
        a.session.group for a in assignments if a.session and a.session.group
    ]
    ...
    for a in assignments:
        if a.session and a.session.group:
            a.session.historic_group = group_replacements.get(
                a.session.group_id)
    ...
    parents = Group.objects.filter(
        pk__in=set(a.session.historic_group.parent_id for a in assignments
                   if a.session and a.session.historic_group and
                   a.session.historic_group.parent_id))
    parent_replacements = find_history_replacements_active_at(
        parents, meeting_time)

    for a in assignments:
        if a.session and a.session.historic_group and a.session.historic_group.parent_id:
            a.session.historic_group.historic_parent = parent_replacements.get(
                a.session.historic_group.parent_id)
...
def read_session_file(type, num, doc):
    ...
    #
    path = os.path.join(settings.AGENDA_PATH, "%s/%s/%s" % (num, type,
                                                            doc.external_url))
...
def agenda_permissions(meeting, schedule, user):
    ...
    if has_role(user, 'Secretariat'):
        ...
        cansee = True
    elif schedule.visible and has_role(
            user, ['Area Director', 'IAB Chair', 'IRTF Chair']):
        cansee = True
...
def session_constraint_expire(request, session):
    ...
    path = reverse(
        session_constraints, args=[session.meeting.number, session.pk])
...
def send_sidemeeting_approval_request(sidemeetingsession):
    ...
    (to_email, cc_list) = gather_address_lists(
        'session_requested', group=group, person=requester)
    from_email = ('"IETF Side Meeting Session Request Tool"',
                  settings.SESSION_REQUEST_DEVELOPERS)
    subject = '{group} - New Side Meeting Request'.format(group=group.acronym)
    template = 'meeting/side/sidemeeting_approval_request.txt'
    url = settings.IDTRACKER_BASE_URL + reverse(
        'side-meeting-approve', kwargs={'pk': meeting.id})
    context = locals()
    send_mail(
        None, to_email, from_email, subject, template, context, cc=cc_list)
...
def can_approve_sidemeeting_request(sidemeeting, user):
    ...
    if group.type.slug == 'wg' and group.parent.role_set.filter(
            name='ad', person=person):
        return True
    if group.type.slug == 'rg' and group.parent.role_set.filter(
            name='chair', person=person):
        return True
    return False
...
def can_request_sidemeeting(user):
    if has_role(user, ('Secretariat', 'Area Director', 'WG Chair',
                       'IRTF Chair', 'RG Chair')):
        return True
    return False
...
def create_interim_meeting(group,
                           date,
                           city='',
                           country='',
                           timezone='UTC',
    ...
    schedule = Schedule.objects.create(
        meeting=meeting, owner=person, visible=True, public=True)
...
def get_announcement_initial(meeting, is_change=False):
    ...
    if group.type.slug == 'rg':
        ...
        type = 'WG'
    initial[
        'subject'] = '{name} ({acronym}) {type} {desc} Meeting: {date}{change}'.format(
            name=group.name,
...
def get_earliest_session_date(formset):
    ...
    return sorted([
        f.cleaned_data['date'] for f in formset.forms
        if f.cleaned_data.get('date')
    ])[0]
...
def get_interim_session_initial(meeting):
    ...
    for session in meeting.session_set.all():
        ...
        if doc:
            ...
            initial['agenda'] = get_document_content(
                os.path.basename(path), path, markup=False)
...
def get_next_agenda_name(meeting):
    ...
    documents = Document.objects.filter(
        type='agenda', session__meeting=meeting)
    if documents:
...
def send_interim_approval_request(meetings):
    ...
    subject = '{group} - New Interim Meeting Request'.format(
        group=group.acronym)
...
def send_interim_announcement_request(meeting):
    ...
    from_email = (settings.SESSION_REQUEST_FROM_EMAIL)
    subject = '{group} - interim meeting ready for announcement'.format(
        group=group.acronym)
    template = 'meeting/interim_announcement_request.txt'
    announce_url = settings.IDTRACKER_BASE_URL + reverse(
        'ietf.meeting.views.interim_request_details',
        kwargs={'number': meeting.number})
    context = locals()
    send_mail(None, to_email, from_email, subject, template, context, cc_list)
...
def send_interim_minutes_reminder(meeting):
    ...
    group = session.group
    (to_email, cc_list) = gather_address_lists(
        'session_minutes_reminder', group=group)
...
def check_interim_minutes():
    ...
    meetings = Meeting.objects.filter(
        type='interim', session__status='sched', date=date)
    for meeting in meetings:
...
def update_interim_session_assignment(form):
    ...
    time = datetime.datetime.combine(form.cleaned_data['date'],
    ...
    if session.official_timeslotassignment():
        ...
        SchedTimeSessAssignment.objects.create(
            timeslot=slot, session=session, schedule=session.meeting.agenda)
