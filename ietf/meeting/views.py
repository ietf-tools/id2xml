...
from ietf.meeting.forms import (
    DETAIL_NAMES, InterimAnnounceForm, InterimCancelForm,
    InterimMeetingModelForm, InterimSessionInlineFormSet,
    InterimSessionModelForm, SideMeetingApproveForm, SideMeetingForm)
from ietf.meeting.helpers import (
    agenda_permissions, build_all_agenda_slices, can_approve_interim_request,
    can_approve_sidemeeting_request, can_edit_interim_request,
    can_edit_sidemeeting_request, can_request_interim_meeting,
    can_request_sidemeeting, can_view_interim_request,
    can_view_sidemeeting_request, convert_draft_to_pdf, find_ads_for_meeting,
    get_all_assignments_from_schedule, get_announcement_initial, get_areas,
    get_earliest_session_date, get_meeting, get_meetings,
    get_modified_from_assignments, get_person_by_email, get_schedule,
    get_schedule_by_name, get_wg_list, get_wg_name_list, is_meeting_approved,
    preprocess_assignments_for_agenda, read_agenda_file, read_session_file,
    send_interim_announcement_request, send_interim_approval_request,
    send_interim_cancellation_notice, send_sidemeeting_approval_request,
    sessions_post_save)
from ietf.meeting.models import (FloorPlan, Meeting, Room, Schedule, Session,
                                 SessionPresentation, SideMeetingSession)
...
from ietf.secr.proceedings.proc_utils import (
    get_progress_stats, import_audio_files, import_youtube_video_urls,
    post_process)
...
def get_menu_entries(request):
    ...
    entries = []
    if has_role(request.user, ('Area Director', 'Secretariat', 'IRTF Chair',
                               'WG Chair', 'RG Chair')):
        entries.append(("Upcoming", reverse("ietf.meeting.views.upcoming")))
        entries.append(("Pending",
                        reverse("ietf.meeting.views.interim_pending")))
        if has_role(request.user, "Secretariat"):
            entries.append(("Announce",
                            reverse("ietf.meeting.views.interim_announce")))
    return entries
...
def send_interim_change_notice(request, meeting):
    ...
    form = InterimAnnounceForm(
        get_announcement_initial(meeting, is_change=True))
...
def materials(request, num=None):
    ...
    past_cutoff_date = datetime.date.today(
    ) > meeting.get_submission_correction_date()

    #sessions  = Session.objects.filter(meeting__number=meeting.number, timeslot__isnull=False)
    schedule = get_schedule(meeting, None)
    sessions = (Session.objects.filter(
        meeting__number=meeting.number, timeslotassignments__schedule=schedule)
                .select_related(
                    'meeting__agenda',
                    'status',
                    'group__state',
    ...
    ietf = sessions.filter(group__parent__type__slug='area').exclude(
        group__acronym='edu')
    irtf = sessions.filter(group__parent__acronym='irtf')
    training = sessions.filter(
        group__acronym__in=['edu', 'iaoc'], type_id__in=[
            'session',
            'other',
        ])
...
def current_materials(request):
    meetings = Meeting.objects.exclude(
        number__startswith='interim-').order_by('-number')
    if meetings:
...
def agenda_create(request, num=None, owner=None, name=None):
    ...
    if schedule is None:
        ...
        messages.error(
            "Error: No meeting information for meeting %s owner %s schedule %s available"
            % (num, owner, name))  # pylint: disable=no-value-for-parameter
    ...
    if not saveasform.is_valid():
        messages.info(request,
                      "This name is not valid. Please choose another one.")
        return redirect(edit_agenda, num=num, owner=owner, name=name)
    ...
    if not ascii_alphanumeric(savedname):
        messages.info(
            request,
            "This name contains illegal characters. Please choose another one."
        )
        return redirect(edit_agenda, num=num, owner=owner, name=name)

    # create the new schedule, and copy the assignments
    try:
        sched = meeting.schedule_set.get(
            name=savedname, owner=request.user.person)
        if sched:
            return redirect(
                edit_agenda,
                num=meeting.number,
                owner=sched.owner_email(),
                name=sched.name)
        else:
    ...
    return redirect(edit_agenda, meeting.number,
                    newschedule.owner_email(), newschedule.name)
...
def edit_timeslots(request, num=None):
    ...
    meeting = get_meeting(num)
    timeslots = meeting.timeslot_set.exclude(location=None).select_related(
        "location", "type")
...
def edit_agenda(request, num=None, owner=None, name=None):
    ...
    if schedule is None:
        raise Http404(
            "No meeting information for meeting %s owner %s schedule %s available"
            % (num, owner, name))
    ...
    rooms = meeting.room_set.filter(
        session_types__slug='session').distinct().order_by("capacity")
    saveas = SaveAsForm()
    saveasurl = reverse(
        edit_agenda,
        args=[meeting.number,
              schedule.owner_email(), schedule.name])

    can_see, can_edit, secretariat = agenda_permissions(
        meeting, schedule, user)
...
#
AgendaPropertiesForm = modelform_factory(
    Schedule, fields=('name', 'visible', 'public'))
...
def edit_agenda_properties(request, num=None, owner=None, name=None):
    ...
    cansee, canedit, secretariat = agenda_permissions(meeting, schedule,
                                                      request.user)
    ...
    if not (canedit or has_role(request.user, 'Secretariat')):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return HttpResponseRedirect(
                    reverse(
                        'ietf.meeting.views.list_agendas', kwargs={'num': num
                                                                   }))
        else:
...
def agenda(request,
    ...
    if not meetings.exists() or (meeting.number.isdigit() and
                                 meeting.number <= 64 and
                                 not meeting.agenda.assignments.exists()):
        if ext == '.html':
            return HttpResponseRedirect(
                'https://www.ietf.org/proceedings/%s' % num)
        else:
            raise Http404
    ...
    seen = set()
    groups = [
        a.session.historic_group for a in filtered_assignments
        if a.session and a.session.historic_group and
        a.session.historic_group.type_id in ('wg', 'rg', 'ag', 'iab') and
        a.session.historic_group.historic_parent
    ]
    group_parents = []
    for g in groups:
...
def agenda_csv(schedule, filtered_assignments):
    ...
    headings = [
        "Date", "Start", "End", "Session", "Room", "Area", "Acronym", "Type",
        "Description", "Session ID", "Agenda", "Slides"
    ]

    def write_row(row):
        encoded_row = [
            v.encode('utf-8') if isinstance(v, unicode) else v for v in row
        ]

        while len(encoded_row) < len(headings):
            # produce empty entries at the end as necessary
    ...
    def agenda_field(item):
        ...
        if agenda_doc:
            return "http://www.ietf.org/proceedings/{schedule.meeting.number}/agenda/{agenda.external_url}".format(
                schedule=schedule, agenda=agenda_doc)
        else:
            return ""

    def slides_field(item):
        return "|".join(
            "http://www.ietf.org/proceedings/{schedule.meeting.number}/slides/{slide.external_url}".
            format(schedule=schedule, slide=slide)
    ...
    for item in filtered_assignments:
        ...
        if item.timeslot.type_id == "break":
            ...
            row.append(item.timeslot.location.name
                       if item.timeslot.location else "")
            row.append("")
            row.append(item.session.historic_group.acronym)
            row.append(
                item.session.historic_group.historic_parent.acronym.upper()
                if item.session.historic_group.historic_parent else "")
            ...
            row.append("")
            row.append(item.session.historic_group.acronym
                       if item.session.historic_group else "")
            row.append("")
            row.append(item.session.name)
            row.append(item.session.pk)
            ...
            row.append(item.session.historic_group.acronym
                       if item.session.historic_group else "")
            row.append("BOF" if item.session.historic_group.state_id in (
                "bof", "bof-conc") else item.session.historic_group.type.name)
            row.append(item.session.historic_group.name
                       if item.session.historic_group else "")
            row.append(item.session.pk)
            row.append(agenda_field(item))
            row.append(slides_field(item))
...
def agenda_by_room(request, num=None, name=None, owner=None):
    ...
    for day in schedule.assignments.dates('timeslot__time', 'day'):
        ss_by_day[day] = []
    for ss in schedule.assignments.order_by(
            'timeslot__location__functional_name', 'timeslot__location__name',
            'timeslot__time'):
        day = ss.timeslot.time.date()
        ss_by_day[day].append(ss)
    return render(request, "meeting/agenda_by_room.html", {
        "meeting": meeting,
        "schedule": schedule,
        "ss_by_day": ss_by_day
    })
...
def agenda_by_type(request, num=None, type=None, name=None, owner=None):
    ...
    if name is None:
        ...
        person = get_person_by_email(owner)
        schedule = get_schedule_by_name(meeting, person, name)
    assignments = schedule.assignments.order_by(
        'session__type__slug', 'timeslot__time', 'session__group__acronym')
    if type:
        assignments = assignments.filter(session__type__slug=type)
    return render(request, "meeting/agenda_by_type.html", {
        "meeting": meeting,
        "schedule": schedule,
        "assignments": assignments
    })
...
def agenda_by_type_ics(request, num=None, type=None):
    meeting = get_meeting(num)
    schedule = get_schedule(meeting)
    assignments = schedule.assignments.order_by('session__type__slug',
                                                'timeslot__time')
    if type:
        assignments = assignments.filter(session__type__slug=type)
    updated = meeting.updated()
...
def session_document(request, num, acronym, type="agenda"):
    ...
    if acronym == "plenaryt":
        d = d.filter(
            session__name__icontains="technical",
            session__slots__type="plenary")
    elif acronym == "plenaryw":
        d = d.filter(
            session__name__icontains="admin", session__slots__type="plenary")
    else:
    ...
    if d:
        ...
        if ext == "txt":
            ...
            content = "<p>Unrecognized %s file '%s'</p>" % (type,
                                                            doc.external_url)
            content = (html5_preamble % doc) + content + html5_postamble

        return HttpResponse(content)

    raise Http404("No %s for the %s session of IETF %s is available" %
                  (type, acronym, num))
...
def session_draft_list(num, session):
    try:
        agendas = Document.objects.filter(
            type="agenda",
...
def session_draft_tarfile(request, num, session):
    ...
    response['Content-Disposition'] = 'attachment; filename=%s-drafts.tgz' % (
        session)
...
def week_view(request, num=None, name=None, owner=None):
    ...
    for a in filtered_assignments:
        # we don't HTML escape any of these as the week-view code is using
        # createTextNode
        item = {
            "key":
            str(a.timeslot.pk),
            "day":
            a.timeslot.time.strftime("%w"),
            "time":
            a.timeslot.time.strftime("%H%M") + "-" +
            a.timeslot.end_time().strftime("%H%M"),
            "duration":
            a.timeslot.duration.seconds,
            "time_id":
            a.timeslot.time.strftime("%m%d%H%M"),
            "dayname":
            "{weekday}, {month} {day_of_month}, {year}".format(
        ...
        if a.session:
            ...
            if a.session.name:
                ...
                if a.session.historic_group.historic_parent:
                    item[
                        "area"] = a.session.historic_group.historic_parent.acronym
...
def room_view(request, num=None, name=None, owner=None):
    ...
    if assignments:
        earliest = assignments.aggregate(
            Min('timeslot__time'))['timeslot__time__min']
        latest = assignments.aggregate(
            Max('timeslot__time'))['timeslot__time__max']

    if unavailable:
        earliest_unavailable = unavailable.aggregate(Min('time'))['time__min']
        if not earliest or (earliest_unavailable and
                            earliest_unavailable < earliest):
    ...
    base_time = earliest
    base_day = datetime.datetime(base_time.year, base_time.month,
                                 base_time.day)
    ...
    for ss in assignments:
        ss.delta_from_beginning = (
            ss.timeslot.time - base_time).total_seconds()
...
def ical_agenda(request, num=None, name=None, ext=None):
    ...
    include = [
        i for i in filter if not (i.startswith('-') or i.startswith('~'))
    ]
    ...
    assignments = schedule.assignments.exclude(
        timeslot__type__in=['lead', 'offagenda'])
    ...
    assignments = [
        ...
        if
        (a.timeslot.type_id in include_types or
         (a.session.historic_group and
          a.session.historic_group.acronym in include) or
         (a.session.historic_group and a.session.historic_group.historic_parent
          and a.session.historic_group.historic_parent.acronym in include)) and
        (not a.session.historic_group or
         a.session.historic_group.acronym not in exclude)
    ]
...
def json_agenda(request, num=None):
    ...
    locations = set()
    parent_acronyms = set()
    assignments = meeting.agenda.assignments.exclude(
    ...
    for asgn in assignments:
        ...
        if asgn.session.historic_group.type_id in [
                'wg',
                'rg',
                'ag',
        ] or asgn.session.historic_group.acronym in [
                'iesg',
        ]:
            sessdict['group'][
                'parent'] = asgn.session.historic_group.historic_parent.acronym
            parent_acronyms.add(
                asgn.session.historic_group.historic_parent.acronym)
        ...
        sessdict['start'] = asgn.timeslot.utc_start_time().strftime(
            "%Y-%m-%dT%H:%M:%SZ")
        ...
        if asgn.session.slides():
            ...
            for slides in asgn.session.slides():
                sessdict['slides'].append(
                    '/api/v1/doc/document/%s/' % slides.name)
            # New alternative
            sessdict['presentations'] = []
            presentations = SessionPresentation.objects.filter(
                session=asgn.session, document__type__slug='slides')
            for pres in presentations:
                sessdict['presentations'].append({
                    ...
                    pres.rev,
                    'resource_uri':
                    '/api/v1/meeting/sessionpresentation/%s/' % pres.id,
                })
        sessdict[
            'session_res_uri'] = '/api/v1/meeting/session/%s/' % asgn.session.id
        sessdict['session_id'] = asgn.session.id
        modified = asgn.session.modified
        for doc in asgn.session.materials.all():
            rev_docevent = doc.latest_event(NewRevisionDocEvent,
                                            'new_revision')
            modified = max(modified, (rev_docevent and rev_docevent.time) or
                           modified)
    ...
    for obj in meetinfo:
        obj['modified'] = tz.localize(obj['modified']).astimezone(
    ...
    if last_modified:
        ...
        response['Last-Modified'] = format_date_time(
            timegm(last_modified.timetuple()))
    return response
...
def meeting_requests(request, num=None):
    ...
    sessions = Session.objects.filter(
        ...
        type__slug='session',
        group__parent__isnull=False).exclude(requested_by=0).order_by(
            "group__parent__acronym", "status__slug", "group__acronym")

    groups_not_meeting = Group.objects.filter(
        state='Active', type__in=['WG', 'RG', 'BOF']).exclude(
            acronym__in=[session.group.acronym
                         for session in sessions]).order_by(
                             "parent__acronym", "acronym")
...
def get_sessions(num, acronym):
    ...
    def sort_key(session):
        official_sessions = session.timeslotassignments.filter(
...
def session_details(request, num, acronym):
    ...
    for session in sessions:
        ...
        ss = session.timeslotassignments.filter(
            schedule=meeting.agenda).order_by('timeslot__time')
        if ss:
            session.time = ', '.join(
                x.timeslot.time.strftime("%A %b-%d-%Y %H%M") for x in ss)
        ...
        session.filtered_artifacts = session.sessionpresentation_set.filter(
            document__type__slug__in=['agenda', 'minutes', 'bluesheets'])
        session.filtered_slides = session.sessionpresentation_set.filter(
            document__type__slug='slides').order_by('order')
        session.filtered_drafts = session.sessionpresentation_set.filter(
            document__type__slug='draft')
        # TODO FIXME Deleted materials shouldn't be in the
        # sessionpresentation_set
        for qs in [
                session.filtered_artifacts, session.filtered_slides,
                session.filtered_drafts
        ]:
            qs = [
                p for p in qs
                if p.document.get_state_slug(p.document.type_id) != 'deleted'
            ]
            session.type_counter.update([p.document.type.slug for p in qs])

    can_manage = can_manage_materials(
        request.user, Group.objects.get(acronym=acronym))
    ...
    def clean(self):
        ...
        if problems:
            raise forms.ValidationError(
                "Already linked: %s" % ', '.join([d.name for d in problems]))
        return self.cleaned_data


def add_session_drafts(request, session_id, num):
    # num is redundant, but we're dragging it along an artifact of where we
    # are in the current URL structure
    ...
    already_linked = [
        sp.document
        for sp in session.sessionpresentation_set.filter(
    ...
    return render(
        request,
        "meeting/add_session_drafts.html", {
            'session':
            session,
            'session_number':
            session_number,
            'already_linked':
            session.sessionpresentation_set.filter(document__type_id='draft'),
            'form':
            form,
        })
...
def upload_session_bluesheets(request, session_id, num):
    ...
    if session.meeting.type.slug == 'ietf' and not has_role(
            request.user, 'Secretariat'):
    ...
    bluesheet_sp = session.sessionpresentation_set.filter(
        document__type='bluesheets').first()
        ...
        if form.is_valid():
            ...
            if bluesheet_sp:
                ...
                if session.meeting.type_id == 'ietf':
                    name = 'bluesheets-%s-%s-%s' % (
                        session.meeting.number, session.group.acronym,
                        sess_time.strftime("%Y%m%d%H%M"))
                    title = 'Bluesheets IETF%s: %s : %s' % (
                        session.meeting.number, session.group.acronym,
                    ...
                    name = 'bluesheets-%s-%s' % (
                        ...
                        sess_time.strftime("%Y%m%d%H%M"))
                    title = 'Bluesheets %s: %s' % (
                        session.meeting.number, sess_time.strftime("%a %H:%M"))
                doc = Document.objects.create(
                    ...
                    rev='00', )
                doc.states.add(
                    State.objects.get(type_id='bluesheets', slug='active'))
...
class UploadMinutesForm(forms.Form):
    file = forms.FileField(
        label=
        'Minutes file to upload. Note that you can only upload minutes in txt, html, or pdf formats.'
    )
    apply_to_all = forms.BooleanField(
        label='Apply to all group sessions at this meeting',
        initial=True,
        required=False)
    ...
    def clean_file(self):
        ...
        if file._size > settings.SECR_MAX_UPLOAD_SIZE:
            raise forms.ValidationError(
                'Please keep filesize under %s. Requested upload size is %s' %
                (filesizeformat(settings.SECR_MAX_UPLOAD_SIZE),
                 filesizeformat(file._size)))
        if os.path.splitext(
                file.name)[1].lower() not in VALID_MINUTES_EXTENSIONS:
            raise forms.ValidationError(
                'Only these file types supported for minutes: %s' %
                ','.join(VALID_MINUTES_EXTENSIONS))
        return file
...
def upload_session_minutes(request, session_id, num):
    ...
    if not session.can_manage_materials(request.user):
        return HttpResponseForbidden(
            "You don't have permission to upload minutes for this session.")
    if session.is_material_submission_cutoff() and not has_role(
            request.user, "Secretariat"):
        return HttpResponseForbidden(
    ...
    minutes_sp = session.sessionpresentation_set.filter(
        document__type='minutes').first()
        ...
        if form.is_valid():
            ...
            if minutes_sp:
                ...
                if session.meeting.type_id == 'ietf':
                    ...
                    name = 'minutes-%s-%s' % (session.meeting.number,
                                              sess_time.strftime("%Y%m%d%H%M"))
                    title = 'Minutes %s: %s' % (session.meeting.number,
                                                sess_time.strftime("%a %H:%M"))
                doc = Document.objects.create(
                    name=name,
                    type_id='minutes',
                    ...
                    rev='00', )
                doc.states.add(
                    State.objects.get(type_id='minutes', slug='active'))
                doc.docalias_set.create(name=doc.name)
                session.sessionpresentation_set.create(document=doc, rev='00')
            if apply_to_all:
                for other_session in sessions:
                    if other_session != session:
                        other_session.sessionpresentation_set.filter(
                            document__type='minutes').delete()
                        other_session.sessionpresentation_set.create(
            ...
            e = NewRevisionDocEvent.objects.create(
                ...
                rev=doc.rev)
            doc.save_with_history([e])
            # The way this function builds the filename it will never trigger
            # the file delete in handle_file_upload.
...
class UploadAgendaForm(forms.Form):
    file = forms.FileField(
        label=
        'Agenda file to upload. Note that you can only upload agendas in txt or html formats.'
    )
    apply_to_all = forms.BooleanField(
        label='Apply to all group sessions at this meeting',
        initial=True,
        ...
        if file._size > settings.SECR_MAX_UPLOAD_SIZE:
            raise forms.ValidationError(
                ...
                (filesizeformat(settings.SECR_MAX_UPLOAD_SIZE),
                 filesizeformat(file._size)))
        if os.path.splitext(
                file.name)[1].lower() not in VALID_AGENDA_EXTENSIONS:
            raise forms.ValidationError(
                'Only these file types supported for agendas: %s' %
                ','.join(VALID_AGENDA_EXTENSIONS))
        return file
...
def upload_session_agenda(request, session_id, num):
    ...
    if not session.can_manage_materials(request.user):
        return HttpResponseForbidden(
            "You don't have permission to upload an agenda for this session.")
    if session.is_material_submission_cutoff() and not has_role(
            request.user, "Secretariat"):
        return HttpResponseForbidden(
            "The materials cutoff for this session has passed. Contact the secretariat for further action."
    ...
    agenda_sp = session.sessionpresentation_set.filter(
        document__type='agenda').first()
        ...
        if form.is_valid():
            ...
            if agenda_sp:
                ...
                if Document.objects.filter(name=name).exists():
                    ...
                    doc.docalias_set.create(name=doc.name)
                doc.states.add(
                    State.objects.get(type_id='agenda', slug='active'))
...
VALID_SLIDE_EXTENSIONS = ('.doc', '.docx', '.pdf', '.ppt', '.pptx',
                          '.txt')  # Note the removal of .zip
        ...
        if file._size > settings.SECR_MAX_UPLOAD_SIZE:
            raise forms.ValidationError(
                ...
                (filesizeformat(settings.SECR_MAX_UPLOAD_SIZE),
                 filesizeformat(file._size)))
        if os.path.splitext(
                file.name)[1].lower() not in VALID_SLIDE_EXTENSIONS:
            raise forms.ValidationError(
                'Only these file types supported for slides: %s' %
                ','.join(VALID_SLIDE_EXTENSIONS))
        return file
...
def upload_session_slides(request, session_id, num, name):
    ...
    if not session.can_manage_materials(request.user):
        return HttpResponseForbidden(
            "You don't have permission to upload slides for this session.")
    if session.is_material_submission_cutoff() and not has_role(
            request.user, "Secretariat"):
        return HttpResponseForbidden(
            "The materials cutoff for this session has passed. Contact the secretariat for further action."
    ...
    if name:
        ...
        if not (slides and slides.type_id == 'slides'):
            raise Http404
        slides_sp = session.sessionpresentation_set.filter(
        ...
        if form.is_valid():
            ...
            if slides_sp:
                ...
                if Document.objects.filter(name=name).exists():
                    ...
                    doc.docalias_set.create(name=doc.name)
                doc.states.add(
                    State.objects.get(type_id='slides', slug='active'))
                doc.states.add(
                    State.objects.get(type_id='reuse_policy', slug='single'))
            if session.sessionpresentation_set.filter(document=doc).exists():
                sp = session.sessionpresentation_set.get(document=doc)
                sp.rev = doc.rev
                sp.save()
            else:
                max_order = session.sessionpresentation_set.filter(
                    document__type='slides').aggregate(
                        Max('order'))['order__max'] or 0
                session.sessionpresentation_set.create(
                    document=doc, rev=doc.rev, order=max_order + 1)
            if apply_to_all:
                for other_session in sessions:
                    if other_session != session and not other_session.sessionpresentation_set.filter(
                            document=doc).exists():
                        max_order = other_session.sessionpresentation_set.filter(
                            document__type='slides').aggregate(
                                Max('order'))['order__max'] or 0
                        other_session.sessionpresentation_set.create(
                            document=doc, rev=doc.rev, order=max_order + 1)
...
def remove_sessionpresentation(request, session_id, num, name):
    ...
    if not session.can_manage_materials(request.user):
        return HttpResponseForbidden(
            "You don't have permission to manage materials for this session.")
    if session.is_material_submission_cutoff() and not has_role(
            request.user, "Secretariat"):
        return HttpResponseForbidden(
            "The materials cutoff for this session has passed. Contact the secretariat for further action."
    ...
    return render(request, 'meeting/remove_sessionpresentation.html',
                  {'sp': sp})
...
def set_slide_order(request, session_id, num, name):
    ...
    if request.method != 'POST' or not request.POST:
        return HttpResponse(
            json.dumps({
                'success': False,
                'error': 'No data submitted or not POST'
            }),
    ...
    try:
        ...
        return HttpResponse(
            json.dumps({
                'success': False,
                'error': 'Supplied order is not valid'
            }),
            content_type='application/json')
...
def make_schedule_official(request, num, owner, name):
    ...
    if request.method == 'POST':
        ...
        meeting.save()
        return HttpResponseRedirect(
            reverse('ietf.meeting.views.list_agendas', kwargs={'num': num}))

    if not schedule.public:
        messages.warning(
            request,
            "This schedule will be made public as it is made official.")

    if not schedule.visible:
        messages.warning(
            request,
            "This schedule will be made visible as it is made official.")
...
def delete_schedule(request, num, owner, name):
    ...
    if schedule.name == 'Empty-Schedule':
        return HttpResponseForbidden(
            'You may not delete the default empty schedule')

    if schedule == meeting.agenda:
        return HttpResponseForbidden(
            'You may not delete the official agenda for %s' % meeting)

    if not (has_role(request.user, 'Secretariat') or
            person.user == request.user):
        return HttpResponseForbidden(
            "You may not delete other user's schedules")
...
def ajax_get_utc(request):
    ...
    if utc_day_offset != 0:
        html = html + \
            "<span class='day-offset'> {0:+d} Day</span>".format(
                utc_day_offset)
    context_data = {
        'timezone': timezone,
        'time': time,
...
def interim_announce(request):
    ...
    meetings = Meeting.objects.filter(
        type='interim', session__status='scheda').distinct()
...
def interim_send_announcement(request, number):
    ...
    if request.method == 'POST':
        form = InterimAnnounceForm(
            request.POST, initial=get_announcement_initial(meeting))
        if form.is_valid():
            message = form.save(user=request.user)
            message.related_groups.add(group)
...
def interim_skip_announcement(request, number):
    ...
    if request.method == 'POST':
        ...
        messages.success(request,
                         'Interim meeting scheduled.  No announcement sent.')
        return redirect(interim_announce)
...
def interim_pending(request):
    ...
    meetings = Meeting.objects.filter(
        type='interim', session__status='apprw').distinct().order_by('date')
    menu_entries = get_menu_entries(request)
    selected_menu_entry = 'pending'

    meetings = [
        m for m in meetings if can_view_interim_request(m, request.user)
    ]
...
def interim_request(request):
    ...
    if request.method == 'POST':
        ...
        form = InterimMeetingModelForm(request=request, initial=initial)
...
def interim_request_cancel(request, number):
    ...
    if not can_view_interim_request(meeting, request.user):
        return HttpResponseForbidden(
            "You do not have permissions to cancel this meeting request")

    if request.method == 'POST':
        form = InterimCancelForm(request.POST)
        if form.is_valid():
            if 'comments' in form.changed_data:
                meeting.session_set.update(
                    agenda_note=form.cleaned_data.get('comments'))
        ...
        form = InterimCancelForm(
            initial={'group': group.acronym,
                     'date': meeting.date})
...
def interim_request_details(request, number):
    ...
    if request.method == 'POST':
        if request.POST.get('approve') and can_approve_interim_request(
        ...
        if request.POST.get('disapprove') and can_approve_interim_request(
                meeting, request.user):
...
def interim_request_edit(request, number):
    ...
    if not can_edit_interim_request(meeting, request.user):
        return HttpResponseForbidden(
            "You do not have permissions to edit this meeting request")
    ...
    if request.method == 'POST':
        form = InterimMeetingModelForm(
            request=request, instance=meeting, data=request.POST)
        ...
        if form.is_valid() and formset.is_valid():
            ...
            if (form.has_changed() or
                    formset.has_changed()) and meeting.session_set.filter(
                        status='sched'):
...
def past(request):
    ...
    meetings = Meeting.objects.filter(date__lte=today).exclude(
        session__status__in=('apprw', 'scheda',
                             'canceledpa')).order_by('-date')

    # extract groups hierarchy for display filter
    seen = set()
    groups = [
        m.session_set.first().group for m in meetings.filter(type='interim')
    ]
    group_parents = []
    for g in groups:
...
def upcoming_ical(request):
    ...
    assignments = []
    for meeting in meetings:
        items = meeting.agenda.assignments.order_by('session__type__slug',
                                                    'timeslot__time')
    ...
    if filters:
        assignments = [
            a for a in assignments
            if a.session.group and (a.session.group.acronym in filters or (
                a.session.group.parent and
...
def proceedings(request, num=None):
    ...
    schedule = get_schedule(meeting, None)
    sessions = Session.objects.filter(meeting__number=meeting.number).filter(
        Q(timeslotassignments__schedule=schedule) | Q(
            status='notmeet')).select_related().order_by('-status_id')
    plenaries = sessions.filter(name__icontains='plenary').exclude(
        status='notmeet')
    ietf = sessions.filter(group__parent__type__slug='area').exclude(
        group__acronym='edu')
    irtf = sessions.filter(group__parent__acronym='irtf')
    training = sessions.filter(
        group__acronym__in=['edu', 'iaoc'], type_id__in=[
            'session',
            'other',
        ]).exclude(status='notmeet')
    iab = sessions.filter(group__parent__acronym='iab').exclude(
        status='notmeet')

    cache_version = Document.objects.filter(
        session__meeting__number=meeting.number).aggregate(
            Max('time'))["time__max"]
...
def finalize_proceedings(request, num=None):
    ...
    return render(request, "meeting/finalize.html", {
        'meeting': meeting,
    })
...
def proceedings_acknowledgements(request, num=None):
    ...
    if int(meeting.number) < settings.NEW_PROCEEDINGS_START:
        return HttpResponseRedirect(
            'https://www.ietf.org/proceedings/%s/acknowledgement.html' % num)
...
def proceedings_attendees(request, num=None):
    ...
    if int(meeting.number) < settings.NEW_PROCEEDINGS_START:
        return HttpResponseRedirect(
            'https://www.ietf.org/proceedings/%s/attendees.html' % num)
...
def proceedings_overview(request, num=None):
    ...
    if int(meeting.number) < settings.NEW_PROCEEDINGS_START:
        return HttpResponseRedirect(
            'https://www.ietf.org/proceedings/%s/overview.html' % num)
...
def proceedings_progress_report(request, num=None):
    ...
    if int(meeting.number) < settings.NEW_PROCEEDINGS_START:
        return HttpResponseRedirect(
            'https://www.ietf.org/proceedings/%s/progress-report.html' % num)
...
class OldUploadRedirect(RedirectView):
    def get_redirect_url(self, **kwargs):
        return reverse_lazy(
            'ietf.meeting.views.session_details', kwargs=self.kwargs)
...
class SideMeetingAddView(CreateView):
    ...
    """
    template_name = 'meeting/side/add.html'
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        return super(SideMeetingAddView, self).dispatch(
            request, *args, **kwargs)
    ...
    def get_context_data(self, **kwargs):
        ...
        context['form'].fields['meeting'].queryset = Meeting.objects.exclude(
            number__icontains='interim')
        return context
    ...
    def form_valid(self, form):
        ...
        if (not form.cleaned_data['group']) or (self.object.meeting.type.name
                                                != "IETF"):
...
class SideMeetingApproveView(UpdateView):
    ...
    """
    template_name = 'meeting/side/approve.html'
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not can_approve_sidemeeting_request(sidemeeting, request.user):
            raise PermissionDenied
        return super(SideMeetingApproveView, self).dispatch(
            request, *args, **kwargs)

    def get_context_data(self, **kwargs):
        context = super(SideMeetingApproveView, self).get_context_data(
            **kwargs)
...
class SideMeetingEditView(UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not can_edit_sidemeeting_request(sidemeeting, request.user):
            raise PermissionDenied
        return super(SideMeetingEditView, self).dispatch(
            request, *args, **kwargs)
...
class SideMeetingListView(ListView):
    ...
    template_name = 'meeting/side/list.html'

    def dispatch(self, request, *args, **kwargs):
        # ensure the user can view the data from the db
        if not can_request_sidemeeting(request.user):
            raise PermissionDenied
        return super(SideMeetingListView, self).dispatch(
            request, *args, **kwargs)
...
class SideMeetingDetailView(DetailView):
    template_name = 'meeting/side/detail.html'
    model = SideMeetingSession
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not can_view_sidemeeting_request(sidemeeting, request.user):
            raise PermissionDenied
        return super(SideMeetingDetailView, self).dispatch(
            request, *args, **kwargs)
