...
urlpatterns = patterns('ietf.submit.views',
    ...
    url(r'^manualpost/listsub$', 'manual_post_email_submissions', name='submit_manualpost_list'),
    ...
    url(r'^manualpost/attachment/(?P<submission_id>\d+)/(?P<message_id>\d+)/(?P<access_token>[a-f\d]*)/(?P<filename>.*)$', 'submission_email_attachment', name='submit_submission_email_attachment_by_hash'),
)
