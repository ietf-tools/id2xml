...
from ietf.doc.mails import send_review_suggested_replaces_request
...
def post_submission(request, submission):
    ...
    new_replaces, new_suggested_replaces = update_replaces_from_submission(request, submission, draft)

    announce_to_lists(request, submission)
    announce_new_version(request, submission, draft, state_change_msg)
    announce_to_authors(request, submission)

    if new_suggested_replaces:
        send_review_suggested_replaces_request(request, draft)
