...
def _main(outfile=sys.stdout):
    global opt_debug, opt_timestamp, opt_trace, opt_authorinfo, opt_getauthors, files, company_domain
    ...
    try:
        opts, files = getopt.gnu_getopt(sys.argv[1:], "dhtTv", ["debug", "getauthors", "help", "timestamp", "notimestamp", "trace", "version",])
    except Exception, e:
        print "%s: %s" % (program, e)
        sys.exit(1)
    ...
    for opt, value in opts:
        if opt in ["-d", "--debug"]:  # Output debug information
            opt_debug = True
        elif opt in ["--getauthors"]:  # Output debug information
            opt_getauthors = True
