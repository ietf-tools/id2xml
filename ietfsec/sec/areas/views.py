    ...
    * none
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('submit', '') == "Retire":
            ...
            # delete telechat_user records that exists
            # should really only be one but this case nicely handles 0, 1 or more
            telechat_records = TelechatUser.objects.filter(person_or_org_tag=tag)
            for record in telechat_records:
                record.delete()
