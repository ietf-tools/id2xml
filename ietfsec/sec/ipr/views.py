...
from sec.ipr.models import IprDetail, IprUpdate, IprRfc, IprDraft, IprContact, LICENSE_CHOICES, STDONLY_CHOICES, IprNotification
...
from sec.utils.mail import send_mail_text
    ...
    if is_draft:
        ...
        if rel.document.group.acronym_id == 1027: 
            try:
                ...
                if draft_internal.job_owner > 0: 
    
                    person = IESGLogin.objects.filter(id=draft_internal.job_owner)
                    if person:
  ...
  Click <a href="./ipr.cgi"> here</a> if you want to disclose information about patents or patent applications where you do have a relationship to the patent owners or patent applicants.'''
   ...
   disclosed patent information relates.'''
    ...
    if not ipr_dtl.generic:
        ...
        if rfcs:
            rfc_titles = [
                rfc.document.title for rfc in rfcs
            ]
            rfcs_data = tuple([
                'RFC Number:',
                [rfc.document.rfc_number for rfc in rfcs]
            ])
        if drafts:
            draft_titles = [
                draft.document.title for draft in drafts
            ]
            drafts_data = tuple([
                'ID Filename:',
                [draft.document.filename+'.txt' for draft in drafts]
            ])
...
def ajax_rfc_num(request):
    ...
    results += list(rfcs)
    response = [ dict(id=r.rfc_number, label=unicode(r.rfc_number)+" "+r.title) for r in results ]
...
def ajax_internet_draft(request):
    ...
    results = InternetDraft.objects.filter(filename__icontains=q)
    if results.count() > 20:
        results = results[:20]
    elif results.count() == 0:
        return { 'success' : False, 'error' : "No results" }

    response = [dict(id=r.id_document_tag, label = r.filename) for r in results]
    return response
