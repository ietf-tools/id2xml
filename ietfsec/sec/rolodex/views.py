...
def get_roles(tag):
    ...
    for item in qs:
        ...
        d['html'] = '<a href="../../groups/%s/">%s</a>' % (item.group_acronym_id, item.group_acronym)
        roles.append(d)
    qs = person.irtfchair_set.all()
    for item in qs:
        d = {} 
        d['label'] = 'IRTF Chair'
        # NOTE: there is currently no IRTF maintenance app to link to so use plain text
        d['html'] = item.irtf.acronym
        roles.append(d)
    return roles
    ...
    * person, emails
    ...
    if request.method == 'POST':
        ...
        if email_form.is_valid():
            ...
            messages.success(request, 'The Bulk Update was successful')
...
def delete(request, tag):
    ...
    * person
	...
	if request.POST.get('post', '') == "yes":
	    ...
	    person.delete()
            messages.success(request, 'The Rolodex entry was deleted successfully')
