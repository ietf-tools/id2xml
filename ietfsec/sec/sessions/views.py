...
from sec.utils.ams_mail import get_ad_email_list, get_chair_email_list, get_cc_list
...
# Helper Functions
# -------------------------------------------------
def clear_not_meeting(group,meeting):
    '''
    Takes a group object and meeting object and clears the not_meeting_groups record.  Called from
    new(), in case the group had previously been marked as not meeting.
    NOTE: this function has to use custom raw SQL because the underlying table, not_meeting_groups,
    has no primary key, so isn't supported by Django.
    '''
    cursor = connection.cursor()
    cursor.execute('DELETE from not_meeting_groups WHERE group_acronym_id=%s and meeting_num=%s' % (group.pk, meeting.pk))
...
def get_meeting():
    '''
    Function to get the current meeting.  Simply returns the highest meeting number in
    the meetings table.
    '''
    return Meeting.objects.all().order_by('-meeting_num')[0]

def get_scheduled_sessions(request, meeting):
    '''
...
def get_my_groups(request):
    '''
    Takes request object (for the user info)
    Returns a list of groups (IETFWG, IRTF) the user is chair or secretary of.
    If the user is a secretariat all active groups (IETFWG and IRTF) are returned
    '''
    user = request.person
    if request.user_is_secretariat:
        querysets = [IETFWG.objects.filter(status=1),
                     IRTF.objects.all()]
    else:
        querysets = [IETFWG.objects.filter(wgchair__person=user,status=1),
                     IETFWG.objects.filter(wgsecretary__person=user,status=1),
                     IRTF.objects.filter(irtfchair__person=user)]
    
    my_groups = [ x for x in itertools.chain(*querysets) ]
    
    return my_groups
    
def get_unscheduled_groups(request, meeting):
    '''
    This function takes a reuqest object (required to identify the USER) and a meeting object.
    It returns a list of active groups (both IETFWG and IRTF) that the user is related to 
    (as chair or secretary) that do not already have sessions requests for the upcoming meeting.
    If the user is a secretariat than all groups are considered.
    '''
    unscheduled = []
    scheduled_sessions = WgMeetingSession.objects.filter(meeting=meeting)
    scheduled_groups_ids = [ s.group_acronym_id for s in scheduled_sessions ]
    my_groups = get_my_groups(request)
    
    for group in my_groups:
        if group.pk not in scheduled_groups_ids:
...
def main(request):
    ...
    if is_locked and not request.user_is_secretariat:
    ...
    scheduled_sessions = get_scheduled_sessions(request, meeting)
    ...
    unscheduled_groups = get_unscheduled_groups(request, meeting)
