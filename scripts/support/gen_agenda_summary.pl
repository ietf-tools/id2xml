...
sub gen_unassigned {
  ...
  my @List = db_select_multiple($sqlStr) or error(1);
  return "" if ($#List < 0);
  my $ret_txt = "$heading\n";
...
sub get_new_returning {
  ...
  my @List_rfc = db_select_multiple($sqlStr_rfc) or error(3);
...
c.id_document_tag = a.id_document_tag and a.intended_status_id = b.intended_status_id
     ...
     my @List2 = db_select_multiple($sqlStr2) or error(4);
     for $array_ref (@List2) {
...
rfc_intend_status b where a.intended_status_id=b.intended_status_id and a.rfc_number=$id_document_tag");
  ...
  if ($num_doc > 1 and 0) { ## Disabled  - don't know why I have written this block
    my $sqlStr2 = qq{select c.id_document_tag,rfc_flag,filename,id_document_name,status_value,c.note,revision from internet_drafts a, id_intended_status b, id_internal c
where c.ballot_id = $ballot_id and c.primary_flag <> 1 and
c.id_document_tag = a.id_document_tag and a.intended_status_id = b.intended_status_id
};

   @List2 = db_select_multiple($sqlStr2) or error($sqlStr2);
   for $array_ref (@List2) {
      my ($id_document_tag,$rfc_flag,$filename,$doc_name,$statusStr,$note,$revision) = @$array_ref;
...
sub get_wg_action {
  ...
  my @List = db_select_multiple("select id,pwg_status_val from pwg_cat where id <> 11 and id <> 21 order by id") or error(6);
  for $array_ref (@List) {
...
sub get_wg_action_text {
  my $sqlStr = shift;
  my $ret_txt = "";
  @List = db_select_multiple($sqlStr) or error(7);
  if ($#List < 0) {
    $ret_txt .= qq {    NONE
};
  } else {
...
sub get_agenda_footer {
  ...
  my $wg_news_txt = "$news_index. Agenda Working Group News\n";
