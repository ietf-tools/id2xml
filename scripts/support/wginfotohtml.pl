...
<p>Additional information is available at <a href="http://tools.ietf.org/wg/$group_acronym">tools.ietf.org/wg/$group_acronym</a>
};
...
#};
  ####### Group Chairs #########
  my $chairs_str = "";
  my @List = db_select_multiple("select person_or_org_tag from g_chairs where group_acronym_id = $group_acronym_id order by id");
  for $array_ref (@List) {
    ...
    #$chairs_str .= " (acting chair)" if ($group_acronym eq 'dime' and $chair_name =~ /Loughney/);
    $chairs_str .= "<br><br>\n";
  }
  ###### Area Directors ########
  $sqlStr = qq {
...
$ad_adv_str
  ...
  if ($#List > -1) {
    ...
    for $array_ref (@List) {
      ...
      $tech_name .= " (security)" if ($group_acronym_id==1600 and ($tech_tag==106567 or $tech_tag==23114));
  ...
  ###### Group Secretary ########
  @List = db_select_multiple("select person_or_org_tag from g_secretaries where group_acronym_id = $group_acronym_id");
  if ($#List > -1) {
...
sub get_html {
  ...
  my $email = shift;
  my $html_txt = "";
#    if ($name eq "Gregory Lebovitz") {
#      $email =~ s/\./dot/g;
#      $email =~ s/\@/at/g;
#      $html_txt .= qq{<li> $name &lt;$email&gt;
#};
#    } elsif ($email) {
    if ($email) {
      $html_txt .= qq {<li> <a href="mailto:$email">$name &lt;$email&gt;</a>
};
    } else {
      $html_txt .= "<li> $name\n";
    }
...
<h2>No Current Internet-Drafts</h2>
  ...
  for $array_ref (@List) {
    ...
    my @stat_file = stat "/a/www/ietf-ftp${text_file}";
...
sub process_rfc {
  ...
  my @List = db_select_multiple("select a.rfc_number,b.status_value,a.rfc_name,a.rfc_published_date from rfcs a, rfc_status b where a.group_acronym = $group_acronym and a.status_id = b.status_id order by a.rfc_published_date");
  my $ret_val = "";
  if ($#List < 0) {
     $ret_val .= qq{
<h2>No Request For Comments</h2>
  ...
  for $array_ref (@List) {
    ...
    $obs_rfc_number = 0 unless my_defined($obs_rfc_number);
    ...
    $filesize = 0 unless my_defined($filesize);
    my $obs_text = "";
    my @obsList = db_select_multiple("select rfc_acted_on from rfcs_obsolete where action='Obsoletes' and rfc_number=$rfc_number");
    if ($#obsList > -1) {
      $obs_text .= "<font color=\"orange\"> obsoletes ";
      for my $array_ref (@obsList) {
        my ($rfc_acted_on) = @$array_ref;
        $obs_text .= "RFC $rfc_acted_on,";
      }
      chop($obs_text);
      $obs_text .= "</font>/"
    }
    my @obsbyList = db_select_multiple("select rfc_number from rfcs_obsolete where action='Obsoletes' and rfc_acted_on=$rfc_number");
    if ($#obsbyList > -1) {
      $obs_text .= "<font color=\"red\"> obsoleted by ";
      for my $array_ref (@obsbyList) {
        my ($rfc_acted_on) = @$array_ref;
        $obs_text .= "RFC $rfc_acted_on,";
      }
      chop($obs_text);
    ...
    my @updList = db_select_multiple("select rfc_acted_on from rfcs_obsolete where action='Updates' and rfc_number=$rfc_number");
    if ($#updList > -1) {
      $obs_text .= "<font color=\"orange\"> updates ";
      for my $array_ref (@updList) {
        my ($rfc_acted_on) = @$array_ref;
        $obs_text .= "RFC $rfc_acted_on,";
      }
      chop($obs_text);
    ...
    my @updbyList = db_select_multiple("select rfc_number from rfcs_obsolete where action='Updates' and rfc_acted_on=$rfc_number");
    if ($#updbyList > -1) {
      $obs_text .= "<font color=\"orange\"> updated by ";
      for my $array_ref (@updbyList) {
        my ($rfc_acted_on) = @$array_ref;
        $obs_text .= "RFC $rfc_acted_on,";
      }
      chop($obs_text);
    ...
    chop ($obs_text) if my_defined($obs_text);
...
sub process_description {
  my $group_acronym = shift;
  my $ret_val_body = "";
  my $filename = "/a/www/ietf/WG_DESCRIPTIONS/${group_acronym}.desc.txt";
