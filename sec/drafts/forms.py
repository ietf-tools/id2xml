...
# Select Choices 
# ---------------------------------------------
IS_CHOICES = list(IDIntendedStatus.objects.values_list('intended_status_id','intended_status'))
SEARCH_IS_CHOICES = IS_CHOICES[:]
SEARCH_IS_CHOICES.insert(0,('',''))
STATUS_CHOICES = list(IDStatus.objects.values_list('status_id', 'status')) 
...
rfc_group_tuples = [(g.group_acronym.acronym,g.group_acronym.acronym) for g in groups]
...
class SearchForm(forms.Form):
    ...
    document_name = forms.CharField(max_length=80,required=False)
