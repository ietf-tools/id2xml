    ...
    * form, awp_formset
    ...
    if request.method == 'POST':
		...
		comments = form.cleaned_data['comments'])
            ...
            create_message(request, 'The Group was created successfully!')
    ...
    * none
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('submit', '') == "Delete":
            ...
            obj = get_model('groups',table)
            instance = obj.objects.get(person=tag,group_acronym=group.group_acronym)
            instance.delete()
            create_message(request, 'The entry was deleted successfully')
	...
	if acronym_form.is_valid() and group_formset.is_valid() and awp_formset.is_valid():
	    ...
	    group_formset.save()
            ...
            if 'primary_area' in group_formset.forms[0].changed_data:
                ...
                area_group = AreaGroup.objects.get(group=group)
                area_group.area = area
                area_group.save()
...
def edit_gm(request, id):
    ...
    * group, formset 
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            formset.save()
            create_message(request, 'The Goals Milestones were changed successfully')
            url = reverse('sec.groups.views.view', kwargs={'id':id})
            return HttpResponseRedirect(url)
    else:
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('submit', '') == "Add":
            ...
            if form.is_valid():
                ...
                role_model = get_model('groups', type)
		obj = role_model(person=person, group_acronym=group)
		obj.save()

                create_message(request, 'New %s added successfully!' % type)
