...
class ChangeStateForm(forms.Form):
    ...
    message = forms.CharField(widget=forms.Textarea, label=mark_safe("Message to<br> Secretariat"), help_text="Leave blank to change state without notifying the Secretariat", required=False)
    comment = forms.CharField(widget=forms.Textarea, help_text="Optional comment for the charter history", required=False)
    note = forms.CharField(widget=forms.Textarea, label="Reason", help_text="Optional reason for chartering, leave blank to skip", required=False)
        ...
        if form.is_valid():
            ...
            if charter_state != charter.get_state():
                ...
                if option in ("initcharter", "recharter"):
                    ...
                    if note:
                        c = DocEvent(type="added_comment", doc=charter, by=login)
                        c.desc = u"Reason for %s: %s" % (
                            "rechartering" if chartering_type == "rechartering" else "chartering",
                            note)
                        c.save()
